...
public:
    ...
    int degree;
    Polyfit();
    Polyfit(std::vector<double> x, std::vector<double> y, int d);
...
Polyfit::Polyfit(std::vector<double> x, std::vector<double> y, int d) {
    this->degree = d;
    this->Weight = polyfit_boost(x, y, d);
}

std::vector<double> Polyfit::predict(std::vector<double> x) {
    std::vector<double> result = polyval(this->Weight, x);
    return result;
}

template <typename T>
std::vector<T> polyfit_boost(const std::vector<T>& xValues, const std::vector<T>& yValues, const int degree, const std::vector<T>& weights)
{
