...
void CellTree::buildTree(std::vector<std::vector<int>> cell_bound_idx, InnerNode *root_node, int child_idx)
{
	...
	else
	{
		...
		for (int i = 0; i < pow(2, MetaData::dim); i++)
		{
			...
			// build()
			vector<double> child_range_bound = splitNodeToSplitedRegion(root_node->range_bound, root_node->split_point, i);
		...
		else if (leaf_node->getKeysNum() > cellSplitTh)
		{
			...
			/*vector<double> split_point;
					for (int j = 0; j < leaf_node.range_bound.size(); j++) {
						split_point.push_back(leaf_node.range_bound[j] / 2);
					}*/
...
vector<array<double, 2> *> &CellTree::pointSearch(array<double, 2> &query, vector<array<double, 2> *> &result, ExpRecorder &exp_Recorder)
{
	...
	if (node.type() == typeid(LeafNode *))
	{
		...
		// exp_Recorder.pointTreeTravelTime += chrono::duration_cast<chrono::nanoseconds>(end_treetravel - start_treetravel).count();
	...
	{
		// auto end_treetravel = chrono::high_resolution_clock::now();
		// auto travel_timeconsume = chrono::duration_cast<chrono::nanoseconds>(end_treetravel - start_treetravel).count();
		// exp_Recorder.pointTreeTravelTime += chrono::duration_cast<chrono::nanoseconds>(end_treetravel - start_treetravel).count();
...
vector<array<double, 2> *> &CellTree::rangeSearch(vector<double> &query, vector<array<double, 2> *> &result, ExpRecorder &exp_Recorder)
{
...
void CellTree::DFSCelltree(vector<double> &query, vector<array<double, 2> *> &result, boost::variant<InnerNode *, LeafNode *, GridNode *, int> root)
{
	/*
		get overlap flag
		dissociate return
	...
	/*
		if (root.type() == typeid(InnerNode *))
		{
			...
			for (auto child : innernode->children)
			{
				if (child.type() == typeid(InnerNode *))
				{
					InnerNode *innerChild = boost::get<InnerNode *>(child);
					int overlapFlag = queryCellRealtion(innerChild->range_bound, query);
					if (overlapFlag == DISSOCIATION)
					{
						continue;
					}
				...
				{
					LeafNode *leafChild = boost::get<LeafNode *>(child);
					int overlapFlag = queryCellRealtion(leafChild->range_bound, query);
					if (overlapFlag == DISSOCIATION)
						continue;
					else if (overlapFlag == CONTAIN)
					{
						for (auto metadata : leafChild->metadataVec)
							result.push_back(metadata.data);
					}
					else if (overlapFlag == INTERSECTION)
					{
				...
				{
					GridNode *gridChild = boost::get<GridNode *>(child);
					int overlapFlag = queryCellRealtion(gridChild->rangeBound, query);
					if (overlapFlag == DISSOCIATION)
						continue;
					else if (overlapFlag == CONTAIN)
					{
						for (auto metadata : gridChild->metadataVec)
							result.push_back(metadata.data);
					}
					else if (overlapFlag == INTERSECTION)
					{
